...
export default function App() {
  ...
  const [ faceDetectionBoxArray, setFaceDetectionBoxArray ] = useState(initialFaceDetectionBoxArray);
  ...
  function calculateFaceDetectionBox(facePositionArray) {
    ...
    facePositionArray.forEach(facePosition => {
      const { top_row, left_col, bottom_row, right_col } = facePosition.region_info.bounding_box;
      const detectionBox = {
  ...
  return (
    <div className='app'>
      <Navigation route={route} changeRoute={changeRoute} resetSession={resetSession} />
      <main className='content center f-column'>
        {
          ...
          : route === 'signIn' ? <SignIn changeRoute={changeRoute} loadUser={loadUser} /> 
          : route === 'register' ? <Register changeRoute={changeRoute} addUser={addUser} />
